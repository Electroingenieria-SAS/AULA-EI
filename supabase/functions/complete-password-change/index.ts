...
function validate(value:string,email:string){
  ...
  if(/\s/.test(value)||!/[a-z]/.test(value)||!/[A-Z]/.test(value)||!/[0-9]/.test(value)||!/[^A-Za-z0-9]/.test(value))return "Usa mayúscula, minúscula, número y símbolo, sin espacios.";
...
serve(async(req)=>{
  ...
  try{
    ...
    const {data:profile}=await admin.from("profiles").select("id,email,is_active").eq("id",user.id).single();
    if(!profile||profile.is_active!==true)return reply({ok:false,error:"Cuenta no activa en Aula EI."},403);
    if(!user.app_metadata?.aula_ei_role)return reply({ok:false,error:"Cuenta no administrada por Aula EI."},403);
    ...
    const {error:updateError}=await admin.auth.admin.updateUserById(user.id,{password,app_metadata:{...user.app_metadata,aula_ei_active:true,aula_ei_must_change_password:false},user_metadata:{...user.user_metadata,must_change_password:false}});
    if(updateError)return reply({ok:false,error:updateError.message},400);
    await admin.from("audit_logs").insert({actor_id:user.id,action:"complete_first_password_change",entity_type:"profile",entity_id:user.id,metadata:{completed_at:new Date().toISOString()}});
    return reply({ok:true,message:"Contraseña actualizada. Inicia sesión nuevamente.",force_relogin:true});
  }catch(error){return reply({ok:false,error:error instanceof Error?error.message:"Error inesperado."},500)}
});