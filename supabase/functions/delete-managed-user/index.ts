...
function normalizeRole(raw: unknown): AppRole {
  const value = String(raw || "colaborador").trim().toLowerCase();
  const map: Record<string, AppRole> = {
    colaborador: "colaborador", learner: "colaborador", estudiante: "colaborador",
    creador_contenido: "creador_contenido", content_creator: "creador_contenido", creador: "creador_contenido",
    revisor: "revisor", reviewer: "revisor",
    admin: "admin", administrador: "admin",
    super_admin: "super_admin", superadmin: "super_admin", "super admin": "super_admin",
  };
  return map[value] || "colaborador";
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!url || !key) return reply({ ok: false, error: "Configuración segura del servidor incompleta." }, 500);
    ...
    if (callerError || !caller) return reply({ ok: false, error: "Sesión inválida o vencida." }, 401);
    ...
    const { data: callerProfile } = await admin
      ...
      .eq("id", caller.id)
      .single();
    if (!callerProfile || callerProfile.is_active !== true) {
      return reply({ ok: false, error: "La cuenta administradora no está activa en Aula EI." }, 403);
    }

    const callerRole = normalizeRole(callerProfile.role);
    if (!["admin", "super_admin"].includes(callerRole)) {
      return reply({ ok: false, error: "Solo Admin o Super Admin pueden administrar accesos." }, 403);
    }
    ...
    const { data: targetProfile, error: targetProfileError } = await admin
      ...
      .eq("id", targetId)
      .single();
    if (targetProfileError || !targetProfile) return reply({ ok: false, error: "Usuario no encontrado en Aula EI." }, 404);

    const targetRole = normalizeRole(targetProfile.role);
    if (rank[callerRole] <= rank[targetRole]) {
      return reply({ ok: false, error: "Solo puedes administrar usuarios con un nivel inferior al tuyo." }, 403);
    }
    ...
    if (targetAuthError || !targetAuth) return reply({ ok: false, error: "No se encontró la cuenta en Authentication." }, 404);
    ...
    if (profileUpdateError) {
      ...
      return reply({ ok: false, error: "No fue posible actualizar el estado del perfil: " + profileUpdateError.message }, 500);
    }
    ...
    await admin.from("audit_logs").insert({
      ...
      metadata: {
        ...
        previous_active: targetProfile.is_active,
        new_active: desiredActive,
      },
    });
    ...
    return reply({
      ...
      message: desiredActive
        ? "Usuario reactivado. Se conservó todo su historial."
        : "Usuario desactivado. Se conservaron matrículas, progreso y certificados.",
    });
  } catch (error) {
