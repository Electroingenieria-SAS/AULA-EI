...
serve(async (req) => {
  ...
  try {
    ...
    if (!await validateLiveAdminSession(admin, caller.id, token)) return reply({ ok:false, code:"MFA_REQUIRED", error:"Tu sesión administrativa no está activa o no tiene MFA verificado. Vuelve a autenticarte." }, 403);
